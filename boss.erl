...
-export([main/1, server/1, server/2, server/3]).
...
main([Arg]) ->
    case string:to_integer(Arg) of
        ...
        _ ->
            start_node("miner"),
            miner:remote_start([Arg])
    end;

main(_) ->
    io:format("Usage: ./boss <number_of_zeros | server_ip>~n").
...
boss_loop(Prefix, K, Next, Max, Workers, WorkSize) ->
    receive
        {request_work, Worker} ->
            if
                Next =< Max ->
                    End = min(Next + WorkSize - 1, Max),

                    Worker ! {work, Prefix, Next, End, K},
